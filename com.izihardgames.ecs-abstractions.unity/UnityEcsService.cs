...
namespace IziHardGames.Apps.ForEcs.Abstractions.ForUnity
{
    public class UnityEcsService : IIziService
    {
        ...
        {
            if (!isRunning) throw new InvalidOperationException($"Service must running! You must call that method after {nameof(IIziApp)}.{nameof(IIziApp.StartAsync)}() is Finished");
